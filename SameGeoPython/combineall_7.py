...
for i in range(6000):
    y_filename='prepared_data/data_y_'+str(i)+'.npy'
    x_filename='prepared_data/data_x_'+str(i)+'.npy'
...
for i in found_indices:
    filename='prepared_data/data_y_'+str(i)+'.npy'
    new_BPR_data = np.array(np.load('prepared_data/data_BPR_y_'+str(i)+'.npy'))
    new_BPR_data = getC(new_BPR_data,SR_poro,vol_cell_SR)
    new_data = np.array(np.load(filename))
    if new_data.shape != (2,30,110,5):
        print(new_data.shape)
    else:
        list_data_Y.append(new_data[0,:,:,:])
        list_data_Y.append(new_data[1,:,:,:])
        list_data_BPR_Y.append(new_BPR_data[0,:,:,:])
        list_data_BPR_Y.append(new_BPR_data[1,:,:,:])
        print(filename)
...
for i in found_indices:
    ...
    filename='prepared_data/data_x_'+str(i)+'.npy'
    new_data = np.array(np.load(filename))
    new_BPR_data = np.array(np.load('prepared_data/data_BPR_x_'+str(i)+'.npy'))
    new_BPR_data = getC(new_BPR_data,LR_poro,vol_cell_LR)
    if new_data.shape != (2,15,55,5):
        print(new_data.shape, filename)
    else:
        list_data_X.append(new_data[0,:,:,:])
        list_data_X.append(new_data[1,:,:,:])
        list_data_BPR_X.append(new_BPR_data[0,:,:,:])
        list_data_BPR_X.append(new_BPR_data[1,:,:,:])
        print(filename)
...
np.save("data_Y_2.npy", data_Y)
np.save("data_X_2.npy", data_X)
np.save("data_Y_BPR_2.npy", data_BPR_Y)
np.save("data_X_BPR_2.npy", data_BPR_X)
