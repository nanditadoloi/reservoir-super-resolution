...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_psnr_epoch, args.psnr_epochs):
        ...
        print("Mass errors:", lr_mass, hr_mass, gt_mass, hs_mass)
        ...
        gan_writer.add_scalar("Test/LRM", lr_mass, epoch + 1)
        gan_writer.add_scalar("Test/HRM", hr_mass, epoch + 1)
...
def train_gan(dataloader: torch.utils.data.DataLoader,
    ...
    policy = ''#'gaussian_noise,translation,cutout'
    ...
    for i, (lr, hr) in enumerate(dataloader):
        ...
        d_loss_real = adversarial_criterion(discriminator(DiffAugment(hr,policy=policy)), real_label)
        d_loss_fake = adversarial_criterion(discriminator(DiffAugment(sr,policy=policy).detach()), fake_label)
        ...
        # Adversarial loss for real and fake images (origin GAN).
        adversarial_loss = adversarial_criterion(discriminator(DiffAugment(sr,policy=policy)), real_label)
